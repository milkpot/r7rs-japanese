...
\newcommand{\lambdaexp}{{\cf lambda} 式}
\newcommand{\Lambdaexp}{{\cf lambda} 式}
...
\newenvironment{schemenoindent}{
  ...
  \newcommand{\unspecified}{{\em{}規定されていない}}
  \newcommand{\scherror}{{\em{}エラー}}
  \setupcode
...
\newcommand{\exprtype}{構文}

\newcommand{\auxiliarytype}{補助構文}
...
\clearpage
\@topnewpage[
    \centerline{\large\bf\uppercase{概念、キーワード、手続き定義の}}
    \centerline{\large\bf\uppercase{アルファベット順の索引}}
    \vskip 1ex \bigskip]
    \markboth{Index}{索引}
    \phantomsection
    \addcontentsline{toc}{chapter}{概念、キーワード、手続き定義の\hfil\penalty0 \hbox{\hspace*{2em} アルファベット順の索引}}
    \bgroup %\small
    \parindent\z@
