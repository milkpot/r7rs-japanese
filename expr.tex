...
手続き呼び出しの\defining{動的有効期間}と呼びます。
Schemeでは、{\cf call-with-current-continuation} (\ref{continuations}~節)
によって、手続き呼び出しが戻った後も、その動的有効期間に入り直すことが出来ます。
そのため、呼び出しの動的有効期間は、連続した単一の時間でない場合があります。

この節では\defining{パラメータオブジェクト}が導入されます。
これは動的有効期間に対して新しい値を束縛できるオブジェクトです。
...
Schemeのプログラムでは{\em マクロ}と呼ばれる新しい派生形式を定義して使うことが出来ます。\mainindex{macro}
プログラムによって定義された形式は以下の構文を持ちます。
\begin{scheme}
(\hyper{keyword} {\hyper{datum}} ...)%
\end{scheme}%
ただし\hyper{keyword}はその形式を唯一に決定する識別子です。
この識別子は、そのマクロの{\em 構文キーワード}\index{syntactic keyword}、
または単に{\em キーワード}\index{keyword}と呼ばれます\index{macro keyword}。
\hyper{datum}の数およびその構文は、その形式に依存します。
...
\end{scheme}
\hyper{keyword}は識別子、
\hyper{transformer spec}は{\cf syntax-rules}のインスタンスで、
