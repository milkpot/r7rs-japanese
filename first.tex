...
J\authorsc{OHN} C\authorsc{OWAN}, \authorsc{および}
...
\authorsc{および} J\authorsc{ONATHAN} R\authorsc{EES}} \\
\multicolumn{3}{c}{\textit{(編, アルゴリズム言語Scheme報告書改$^{\mathit{5}}$)}} \\
\\
\multicolumn{3}{c}{M\authorsc{ICHAEL} S\authorsc{PERBER},
R. K\authorsc{ENT} D\authorsc{YBVIG}, M\authorsc{ATTHEW} F\authorsc{LATT},
\authorsc{および} A\authorsc{NTON} \authorsc{VAN} S\authorsc{TRAATEN}} \\
\multicolumn{3}{c}{\textit{(編, アルゴリズム言語Scheme報告書改$^{\mathit{6}}$)}} \\
\end{tabular}
$$
\vskip 2ex
...
この報告書は、プログラミング言語Schemeの定義的記述を記載しています。
...
\vest \ref{formalchapter}~章では、拡張BNF記法で記述されたSchemeの正式な構文を、
その表示的意味論と共に掲載します。
...
標準化されているけれどもオプション扱いの、処理系の特徴名の一覧を掲載します。
