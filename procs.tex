...
{\cf /}の最初以外の引数のいずれかが正確なゼロの場合はエラーです。
最初の引数が正確なゼロであり、他の引数のいずれかがNaNでなければ、
正確なゼロを返す場合があります。
...
(後述)を計算します(処理形が複素数をサポートしていない場合でも)。
