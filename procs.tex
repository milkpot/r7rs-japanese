...
しかし{\cf (/~3~4)}が数学的に不正確な値である{\cf 0}を返すようなことは許容されません。
...
が真\index{真}または偽\index{偽}として扱うオブジェクトであるということです。
...
ただしその\var{producer}の呼び出し元の継続に値が渡されると、
...
処理系はdatum表現を持たないレコード型や他の型を表現するために
構文を拡張しても構いません。
...
特定のjiffyは処理系によっては不適切なサイズとなる可能性があります。
