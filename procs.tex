...
これらの手続きは、引数に対して
Unicode full string uppercasing, lowercasing, case-folding algorithm
を適用し、その結果を返します。
場合によっては、結果の長さが引数と異なることもあります。
結果が引数と{\cf string=?}の意味で同じ場合、その引数が返される場合があります。
ちなみに、言語固有のマッピングおよび folding は用いられません。
...
Schemeの他のどんな手続きとも同様に無制限の有効期間を持ちます。
...
\var{thunk}の呼び出しの動的有効期間に入るときは必ず\var{before}が呼ばれ、
その動的有効期間を出るときは必ず\var{after}が呼ばれます。
手続き呼び出しの動的有効期間とは、その呼び出しが開始されてから戻るまでの間の期間です。
\var{before}サンクおよび\var{after}サンクは
{\cf dynamic-wind}の呼び出し元と同じ動的環境で呼ばれます。
Schemeには{\cf call-with-current-continuation}が存在するため、
呼び出しの動的有効期間は連続した単一の時間でない場合があります。
これは以下のように定義されます。
\begin{itemize}
\item 呼ばれた手続きの本体の実行が開始されるとき、その動的有効期間に入ります。

\item 動的有効期間中に({\cf call-with-current-continuation}を用いて)
捕捉された継続をその動的有効期間外で呼んだときも、
その動的有効期間に入ります。

\item 呼ばれた手続きから戻るとき、その動的有効期間から抜けます。

\item 動的有効期間外で捕捉された継続をその動的有効期間中に呼んだときも、
その動的有効期間から抜けます。
\end{itemize}

\var{thunk}の呼び出しの動的有効期間中に
...
\todo{consider removing unspecified effect}
捕捉した継続を用いて
\var{before}や\var{after}の呼び出しの動的有効期間を出入りした場合の効果は
規定されていません。
